...
import { Home, ClipboardList, TrendingUp, LogOut, HelpCircle, ShoppingBag, LogIn, UserPlus, User } from 'lucide-react';
...
export function PlayfulLayout({ children }: PlayfulLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-playful-bg flex flex-col font-sans overflow-x-hidden">
      <nav className="sticky top-0 z-50 bg-white border-b-4 border-playful-dark px-4 py-3 sm:px-6 lg:px-8 shadow-sm">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 lg:py-12 pb-28 md:pb-12">
        {children}
      </main>
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t-4 border-playful-dark z-[50] px-2 py-3 flex justify-around items-center">
        {navItems.map((item) => (
          <Link
            key={item.path}
            to={item.path}
            className={cn(
              "flex flex-col items-center gap-1 p-2 rounded-xl transition-all duration-200 active:scale-90",
              location.pathname === item.path
        ...
        {isLoggedIn && (
          <button
            onClick={handleLogout}
            className="flex flex-col items-center gap-1 p-2 rounded-xl text-playful-red/60 active:scale-90"
          >
