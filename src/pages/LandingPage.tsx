...
import { Zap, Target, Trophy, MessageSquare, ShieldCheck, Rocket } from 'lucide-react';
    ...
    <div className="space-y-24 pb-32">
      ...
      <section className="space-y-16 py-12 px-4 max-w-7xl mx-auto">
        <div className="text-center space-y-4">
          ...
          <p className="font-bold text-muted-foreground text-xl">Sadece bir uygulama değil, bir ekosistem.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <PlayfulCard key={i} className="bg-white space-y-6 group border-playful-dark shadow-playful hover:rotate-2 transition-all">
              <div className={`${f.color} w-16 h-16 rounded-2xl border-4 border-playful-dark flex items-center justify-center text-white shadow-playful group-hover:-translate-y-2 transition-transform`}>
      ...
      <section className="px-4 max-w-6xl mx-auto">
        <PlayfulCard className="bg-playful-dark text-white p-12 md:p-20 text-center space-y-10 border-playful-dark shadow-playful relative overflow-hidden">
          ...
          <div className="relative flex flex-col sm:flex-row gap-6 justify-center pt-8">
            <Link to="/signup?role=öğrenci" className="playful-button bg-playful-teal text-white text-2xl py-6 px-12 group">
              Öğrenci Olarak Katıl <Rocket className="w-8 h-8 group-hover:animate-bounce" />
            </Link>
            <Link to="/signup?role=koç" className="playful-button bg-playful-red text-white text-2xl py-6 px-12">
              Koç Olarak Katıl <MessageSquare className="w-8 h-8" />
            </Link>
          </div>
        </PlayfulCard>
      </section>
      ...
      <footer className="pt-20 border-t-8 border-playful-dark text-center space-y-8 px-4">
        <div className="text-5xl font-black tracking-tighter uppercase italic bg-playful-yellow border-4 border-playful-dark inline-block px-6 py-2 shadow-playful">TYT KAMPÜS</div>
        <div className="flex flex-wrap gap-x-12 gap-y-4 justify-center font-black text-xl uppercase tracking-tighter text-muted-foreground">
        ...
        <div className="max-w-md mx-auto h-2 bg-playful-dark rounded-full opacity-10" />
        <p className="text-sm font-black text-muted-foreground uppercase tracking-widest pb-10">
